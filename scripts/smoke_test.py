...
class SmokeTest:
    def __init__(self, base_url: str, api_key: str | None, *, verbose: bool = False, json_output: bool = False):
    ...
    def _run_test(self, test_fn):
        ...
        try:
            test_fn()
        except (httpx.ReadError, httpx.WriteError, httpx.CloseError,
                httpx.ConnectError, httpx.ConnectTimeout, httpx.ReadTimeout,
                httpx.PoolTimeout) as exc:
            self.check(f"{name}: network error", False, f"{type(exc).__name__}: {exc}")
        except Exception as exc:
            self.check(f"{name}: unexpected error", False, f"{type(exc).__name__}: {exc}")
        finally:
    ...
    def run(self):
        ...
        try:
            ...
            if self.json_output:
                print(json.dumps({"error": f"Cannot connect to {self.base}: {exc}", "passed": 0, "failed": 1, "skipped": 0}))
            else:
        ...
        if self.json_output:
            ...
            if self.failed == 0:
                ...
                print(f"  {self.passed}/{total} passed, {self.failed} FAILED, {self.skipped} skipped  [{elapsed:.1f}s]")
    ...
    def test_health(self):
        ...
        data = r.json()
        self.check("Health check", r.status_code == 200 and data.get("status") == "ok",
    ...
    def test_version(self):
        ...
        self.check("Version: has version string", bool(data.get("version")),
                    f"version={data.get('version')}")

    def test_tool_descriptions(self):
        """GET /api/tool-descriptions returns all 10 canonical tool descriptions."""
        r = self.client.get(f"{self.api}/tool-descriptions")
        data = r.json()
        expected_tools = {
            "memclaw_recall", "memclaw_write", "memclaw_manage",
            "memclaw_list", "memclaw_doc", "memclaw_entity_get",
            "memclaw_tune", "memclaw_insights", "memclaw_evolve",
        }
        self.check("Tool descriptions: 200", r.status_code == 200, f"status={r.status_code}")
        self.check(f"Tool descriptions: all {len(expected_tools)} tools present",
                    set(data.keys()) == expected_tools,
                    f"keys={set(data.keys())}")
        self.check("Tool descriptions: values are strings",
    ...
    def test_write_with_enrichment(self):
        ...
        r = self.client.post(f"{self.api}/memories", json={
            "tenant_id": TENANT, "agent_id": AGENT,
            "content": "We decided to use PostgreSQL 16 instead of MySQL for the new analytics service.",
        })
        data = r.json()
        self.check("Write memory", r.status_code in (200, 201),
                    f"status={r.status_code}{self._verbose_body(r)}")
        self.check("LLM inferred type", data.get("memory_type") in ("decision", "fact"),
                    f"type={data.get('memory_type')}")
        self.check("LLM generated title", bool(data.get("title")),
                    f"title={data.get('title')}")
        self.check("LLM generated tags", bool(data.get("metadata", {}).get("tags")),
    ...
    def test_temporal_resolution(self):
        ...
        data = r.json()
        self.check("Temporal: ts_valid_end populated",
    ...
    def test_pii_detection(self):
        ...
        self.check("PII: contains_pii=true", meta.get("contains_pii") is True,
                    f"contains_pii={meta.get('contains_pii')}")
        self.check("PII: types detected", len(meta.get("pii_types", [])) > 0,
    ...
    def test_extract_only(self):
        ...
        r = self.client.post(f"{self.api}/memories", json={
            "tenant_id": TENANT, "agent_id": AGENT,
            "content": content, "persist": False,
        })
        data = r.json()
        self.check("Extract-only: returns data", r.status_code in (200, 201) and bool(data.get("id")),
                    f"status={r.status_code}")
        self.check("Extract-only: has title", bool(data.get("title")),
                    f"title={data.get('title')}")

        # Verify NOT in DB: search for the unique content
        sr = self.client.post(f"{self.api}/search", json={
            "tenant_id": TENANT, "query": content, "limit": 1,
        })
        results = sr.json()
        # The unique content shouldn't match anything with high similarity
        found = any(content in m.get("content", "") for m in results) if isinstance(results, list) else False
    ...
    def test_bulk_write(self):
        ...
        r = self.client.post(f"{self.api}/memories/bulk", json={
            "tenant_id": TENANT, "agent_id": AGENT,
            "items": items,
        })
        self.check("Bulk write: 200", r.status_code == 200,
                    f"status={r.status_code}{self._verbose_body(r)}")
        if r.status_code != 200:
            return
        data = r.json()
        self.check("Bulk write: created count", data.get("created", 0) >= 1,
                    f"created={data.get('created')}")
        self.check("Bulk write: has results array",
                    isinstance(data.get("results"), list) and len(data["results"]) == 3,
                    f"results_len={len(data.get('results', []))}")
        self.check("Bulk write: has bulk_ms timing", isinstance(data.get("bulk_ms"), int),
    ...
    def test_search(self):
        ...
        def do_search():
            return self.client.post(f"{self.api}/search", json={
                "tenant_id": TENANT, "query": "PostgreSQL analytics database decision",
            })

        r = _retry_until(do_search,
                         predicate=lambda resp: isinstance(resp.json(), list) and len(resp.json()) > 0,
                         max_wait=5.0)
        data = r.json()
        self.check("Search: returns results", isinstance(data, list) and len(data) > 0,
                    f"count={len(data) if isinstance(data, list) else 'not a list'}")
        if isinstance(data, list) and data:
            self.check("Search: has similarity score", data[0].get("similarity") is not None,
    ...
    def test_min_similarity_filtering(self):
        ...
        r = self.client.post(f"{self.api}/search", json={
            "tenant_id": TENANT,
            "query": "quantum entanglement in superconducting qubits at absolute zero",
        })
        data = r.json()
        self.check("Min similarity: irrelevant query returns few/no results",
                    isinstance(data, list) and len(data) == 0,
                    f"count={len(data) if isinstance(data, list) else 'not a list'}")

    def test_recall(self):
        """POST /api/recall returns LLM-synthesized summary."""
        r = self.client.post(f"{self.api}/recall", json={
            "tenant_id": TENANT,
            "query": "PostgreSQL analytics database",
        })
        data = r.json()
        self.check("Recall: 200", r.status_code == 200, f"status={r.status_code}")
        self.check("Recall: has summary", bool(data.get("summary")),
                    f"summary={data.get('summary', '')[:80]}")
        self.check("Recall: has memory_count", isinstance(data.get("memory_count"), int),
                    f"memory_count={data.get('memory_count')}")
        self.check("Recall: has recall_ms", isinstance(data.get("recall_ms"), int),
                    f"recall_ms={data.get('recall_ms')}")
        self.check("Recall: has memories list", isinstance(data.get("memories"), list),
    ...
    def test_graph_retrieval(self):
        ...
        r_a = self.client.post(f"{self.api}/entities/upsert", json={
            "tenant_id": TENANT, "entity_type": "project",
            "canonical_name": "project atlas",
        })
        r_b = self.client.post(f"{self.api}/entities/upsert", json={
            "tenant_id": TENANT, "entity_type": "person",
            "canonical_name": "john doe",
        })
        if r_a.status_code != 200 or r_b.status_code != 200:
            self.check("Graph: create entities", False,
        ...
        r_rel = self.client.post(f"{self.api}/relations/upsert", json={
            "tenant_id": TENANT,
            "from_entity_id": john_id,
            "relation_type": "works_on",
            "to_entity_id": atlas_id,
        })
        self.check("Graph: relation created", r_rel.status_code == 200,
        ...
        r_mem = self.client.post(f"{self.api}/memories", json={
            "tenant_id": TENANT, "agent_id": AGENT, "fleet_id": graph_fleet,
            "content": content,
        ...
        if r_mem.status_code in (200, 201) and r_mem.json().get("id"):
            ...
            self.check("Graph: write linked memory", False, f"status={r_mem.status_code}")
            return

        # 4. Wait for the memory to be searchable (embedding indexing)
        _retry_until(
            lambda: self.client.post(f"{self.api}/search", json={
                "tenant_id": TENANT, "fleet_ids": [graph_fleet],
                "query": "backend migration",
            }),
            predicate=lambda resp: resp.status_code == 200 and isinstance(resp.json(), list) and len(resp.json()) > 0,
            max_wait=10.0,
        )
        ...
        def do_graph_search():
            return self.client.post(f"{self.api}/search", json={
                "tenant_id": TENANT, "fleet_ids": [graph_fleet], "query": "project atlas",
            })

        r_search = _retry_until(do_graph_search,
                                predicate=lambda resp: resp.status_code == 200 and isinstance(resp.json(), list) and mem_id in [m.get("id") for m in resp.json()],
                                max_wait=12.0)
        ...
        self.check("Graph: 1-hop memory found via relation", found,
    ...
    def test_memory_update(self):
        ...
        self.check("Update: weight change", r.status_code == 200, f"status={r.status_code}")
        if r.status_code == 200:
            self.check("Update: weight applied", r.json().get("weight") == 0.95,
        ...
        self.check("Update: content change", r.status_code == 200, f"status={r.status_code}")
        if r.status_code == 200:
            self.check("Update: content applied", r.json().get("content") == new_content,
                        f"content mismatch")
            self.check("Update: title applied", r.json().get("title") == "Smoke Test Updated",
        ...
        self.check("Update: empty body returns 400", r.status_code == 400, f"status={r.status_code}")
        ...
        self.check("Update: missing memory returns 404", r.status_code == 404, f"status={r.status_code}")
    ...
    def test_status_update(self):
        ...
        r = self.client.patch(f"{self.api}/memories/{mid}/status",
                              params={"tenant_id": TENANT},
                              json={"status": "confirmed"})
        self.check("Status update: 200", r.status_code == 200, f"status={r.status_code}")
        if r.status_code == 200:
            # Verify the status changed
            r2 = self.client.get(f"{self.api}/memories/{mid}", params={"tenant_id": TENANT})
            if r2.status_code == 200:
                self.check("Status update: confirmed",
                            r2.json().get("status") == "confirmed",
                            f"status={r2.json().get('status')}")
            # Revert to active for other tests
            self.client.patch(f"{self.api}/memories/{mid}/status",
                              params={"tenant_id": TENANT},
                              json={"status": "active"})
    ...
    def test_dedup(self):
        ...
        content = "We decided to use PostgreSQL 16 instead of MySQL for the new analytics service."
        r = self.client.post(f"{self.api}/memories", json={
            "tenant_id": TENANT, "agent_id": AGENT, "content": content,
        })
        self.check("Dedup: 409 on duplicate", r.status_code == 409,
                    f"status={r.status_code}")
    ...
    def test_contradiction(self):
        ...
        er = self.client.post(f"{self.api}/entities/upsert", json={
            "tenant_id": TENANT, "entity_type": "technology",
        ...
        if not entity_id:
            self.check("Contradiction: entity created", False, f"status={er.status_code}")
            return

        # Write original triple (runs_on is a single-value predicate)
        r1 = self.client.post(f"{self.api}/memories", json={
            "tenant_id": TENANT, "agent_id": AGENT,
            "content": "Analytics DB runs on PostgreSQL",
            "subject_entity_id": entity_id, "predicate": "runs_on", "object_value": "PostgreSQL",
        })
        self.check("Contradiction: first triple written", r1.status_code in (200, 201),
        ...
        r2 = self.client.post(f"{self.api}/memories", json={
            "tenant_id": TENANT, "agent_id": AGENT,
            "content": "Analytics DB runs on ClickHouse",
            "subject_entity_id": entity_id, "predicate": "runs_on", "object_value": "ClickHouse",
        })
        self.check("Contradiction: second triple written", r2.status_code in (200, 201),
        ...
        def check_old_status():
            return self.client.get(f"{self.api}/memories/{old_id}",
                                   params={"tenant_id": TENANT})

        r_old = _retry_until(check_old_status,
                             predicate=lambda resp: resp.status_code == 200 and resp.json().get("status") == "outdated",
                             interval=1.0, max_wait=15.0)
        if r_old.status_code == 200:
            self.check("Contradiction: old memory marked outdated",
                        r_old.json().get("status") == "outdated",
                        f"status={r_old.json().get('status')}")
        else:
            self.check("Contradiction: fetch old memory", False,
                        f"status={r_old.status_code}")
    ...
    def test_contradiction_chain(self):
        ...
        self.check("Contradiction chain: 200", r.status_code == 200, f"status={r.status_code}")
        if r.status_code == 200:
            data = r.json()
            self.check("Contradiction chain: has superseded data",
    ...
    def test_agent_trust_levels(self):
        ...
        r = self.client.post(f"{self.api}/memories", json={
            "tenant_id": TENANT, "agent_id": trust_agent, "fleet_id": fleet_a,
            "content": f"Trust level test memory {uuid.uuid4().hex[:8]}",
        })
        self.check("Trust: write to home fleet", r.status_code in (200, 201),
                    f"status={r.status_code}")
        if r.status_code in (200, 201) and r.json().get("id"):
            self.memory_ids.append(r.json()["id"])

        # 2. Verify agent was auto-created
        r = self.client.get(f"{self.api}/agents/{trust_agent}", params={"tenant_id": TENANT})
        self.check("Trust: agent auto-registered", r.status_code == 200,
                    f"status={r.status_code}")
        if r.status_code == 200:
            self.check("Trust: default level is 1", r.json().get("trust_level") == 1,
                        f"trust_level={r.json().get('trust_level')}")
            self.check("Trust: home fleet set", r.json().get("fleet_id") == fleet_a,
                        f"fleet_id={r.json().get('fleet_id')}")

        # 3. Cross-fleet search at level 1 -> 403 (only with tenant-scoped key; admin key bypasses)
        r = self.client.post(f"{self.api}/search", json={
            "tenant_id": TENANT, "filter_agent_id": trust_agent, "fleet_ids": [fleet_b],
            "query": "anything",
        })
        # Admin key bypasses trust enforcement, so 200 is acceptable
        self.check("Trust: cross-fleet search denied at level 1",
                    r.status_code in (403, 200),
                    f"status={r.status_code} (200 expected with admin key)")

        # 4. Promote to level 2 (cross_fleet read)
        r = self.client.patch(f"{self.api}/agents/{trust_agent}/trust",
                              params={"tenant_id": TENANT},
                              json={"trust_level": 2})
        self.check("Trust: promote to level 2", r.status_code == 200,
                    f"status={r.status_code}")

        # 5. Cross-fleet search now succeeds
        r = self.client.post(f"{self.api}/search", json={
            "tenant_id": TENANT, "filter_agent_id": trust_agent, "fleet_ids": [fleet_b],
            "query": "anything", "top_k": 1,
        })
        self.check("Trust: cross-fleet search allowed at level 2", r.status_code == 200,
                    f"status={r.status_code}")
        ...
        if self.memory_ids:
            ...
            self.check("Trust: delete at level 2",
        ...
        r = self.client.patch(f"{self.api}/agents/{trust_agent}/trust",
                              params={"tenant_id": TENANT},
                              json={"trust_level": 3})
        self.check("Trust: promote to level 3", r.status_code == 200,
                    f"status={r.status_code}")
        ...
        if self.memory_ids:
            ...
            self.check("Trust: delete allowed at level 3", r.status_code == 204,
                        f"status={r.status_code}")
            if r.status_code == 204:
        ...
        self.check("Trust: list agents", r.status_code == 200 and isinstance(r.json(), list),
                    f"status={r.status_code}")
    ...
    def test_tune(self):
        ...
        r = self.client.post(f"{self.api}/memories", json={
            "tenant_id": TENANT, "agent_id": tune_agent,
            "content": f"Tune test memory for search profile validation {uuid.uuid4().hex[:8]}",
        })
        self.check("Tune: write seed memory", r.status_code in (200, 201),
                    f"status={r.status_code}")
        if r.status_code in (200, 201) and r.json().get("id"):
            self.memory_ids.append(r.json()["id"])

        # 2. Set search profile via tune endpoint
        r = self.client.patch(f"{self.api}/agents/{tune_agent}/tune",
                              params={"tenant_id": TENANT},
        ...
        if r.status_code == 200:
            ...
            self.check("Tune: top_k stored", profile.get("top_k") == 10,
                        f"top_k={profile.get('top_k')}")
            self.check("Tune: min_similarity stored", profile.get("min_similarity") == 0.2,
                        f"min_similarity={profile.get('min_similarity')}")
            self.check("Tune: graph_max_hops stored", profile.get("graph_max_hops") == 0,
                        f"graph_max_hops={profile.get('graph_max_hops')}")

        # 3. Verify profile visible on agent GET
        r = self.client.get(f"{self.api}/agents/{tune_agent}", params={"tenant_id": TENANT})
        self.check("Tune: agent GET has profile",
                    r.status_code == 200 and r.json().get("search_profile", {}).get("top_k") == 10,
        ...
        def do_tune_search():
            return self.client.post(f"{self.api}/search", json={
                "tenant_id": TENANT, "filter_agent_id": tune_agent,
                "query": "tune test memory search profile",
            })

        r = _retry_until(do_tune_search,
                         predicate=lambda resp: resp.status_code == 200,
                         max_wait=5.0)
        self.check("Tune: search with profile succeeds", r.status_code == 200,
                    f"status={r.status_code}")
        ...
        if r.status_code == 200:
            ...
            self.check("Tune: merge preserves top_k", profile.get("top_k") == 10,
                        f"top_k={profile.get('top_k')}")
            self.check("Tune: merge adds similarity_blend", profile.get("similarity_blend") == 0.6,
        ...
        r = self.client.patch(f"{self.api}/agents/{tune_agent}/tune",
                              params={"tenant_id": TENANT},
                              json={"top_k": 999})
        self.check("Tune: rejects out-of-range", r.status_code == 422,
                    f"status={r.status_code}")
    ...
    def test_crystallizer(self):
        ...
        r = self.client.post(f"{self.api}/crystallize", json={
            "tenant_id": TENANT,
        })
        self.check("Crystallizer: trigger", r.status_code == 200,
                    f"status={r.status_code}")
        if r.status_code != 200:
            return
        report_id = r.json().get("report_id")
        self.check("Crystallizer: returns report_id", bool(report_id),
                    f"report_id={report_id}")

        # 2. Poll for completion with retry
        def poll_report():
            return self.client.get(f"{self.api}/crystallize/reports/{report_id}",
                                   params={"tenant_id": TENANT})

        rr = _retry_until(poll_report,
                          predicate=lambda resp: resp.status_code == 200 and resp.json().get("status") in ("completed", "failed"),
                          interval=2.0, max_wait=30.0)
        completed = rr.status_code == 200 and rr.json().get("status") in ("completed", "failed")
        ...
        self.check("Crystallizer: has summary", "overall_score" in (report.get("summary") or {}),
                    f"summary={report.get('summary')}")
        self.check("Crystallizer: has hygiene", bool(report.get("hygiene")),
                    f"hygiene keys={list((report.get('hygiene') or {}).keys())}")
        self.check("Crystallizer: has crystallization", report.get("crystallization") is not None,
                    f"crystallization={report.get('crystallization')}")

        # 4. Verify latest endpoint
        rl = self.client.get(f"{self.api}/crystallize/latest",
                              params={"tenant_id": TENANT})
        self.check("Crystallizer: latest endpoint", rl.status_code == 200,
                    f"status={rl.status_code}")

        # 5. Verify reports list
        rlist = self.client.get(f"{self.api}/crystallize/reports",
                                 params={"tenant_id": TENANT})
        self.check("Crystallizer: reports list", rlist.status_code == 200
                    and isinstance(rlist.json(), list) and len(rlist.json()) > 0,
    ...
    def test_ingest(self):
        ...
        r = self.client.post(f"{self.api}/ingest/preview", json={
            "tenant_id": TENANT,
        ...
        self.check("Ingest preview: 200", r.status_code == 200, f"status={r.status_code}")
        if r.status_code != 200:
            return
        data = r.json()
        facts = data.get("facts", [])
        self.check("Ingest preview: extracted facts", len(facts) > 0,
                    f"fact_count={len(facts)}")

        # 2. Commit the first fact (if any)
        if facts:
            r2 = self.client.post(f"{self.api}/ingest/commit", json={
                "tenant_id": TENANT,
                "fleet_id": "ingest-test",
                "agent_id": AGENT,
                "facts": facts[:2],  # commit up to 2
            })
            self.check("Ingest commit: 200", r2.status_code == 200, f"status={r2.status_code}")
            if r2.status_code == 200:
                commit_data = r2.json()
                self.check("Ingest commit: memories created",
    ...
    def test_fleets_and_stats(self):
        ...
        if r.status_code == 200:
            data = r.json()
            self.check("Fleets: returns list", isinstance(data, list),
        ...
        if r.status_code == 200:
            data = r.json()
            self.check("Memory stats: has by_type", "by_type" in data or "by_status" in data,
                        f"keys={list(data.keys())}")

    def test_fleet_heartbeat(self):
        """POST /api/fleet/heartbeat: register a fleet node."""
        r = self.client.post(f"{self.api}/fleet/heartbeat", json={
            "tenant_id": TENANT,
        ...
        self.check("Fleet heartbeat: 200", r.status_code == 200,
                    f"status={r.status_code}{self._verbose_body(r)}")
        if r.status_code == 200:
            data = r.json()
            self.check("Fleet heartbeat: ok=true", data.get("ok") is True,
                        f"ok={data.get('ok')}")
            self.check("Fleet heartbeat: has node_id", bool(data.get("node_id")),
                        f"node_id={data.get('node_id')}")
            self.check("Fleet heartbeat: has commands list",
    ...
    def test_graph_endpoint(self):
        ...
        if r.status_code == 200:
            data = r.json()
            self.check("Graph: has nodes", "nodes" in data,
                        f"keys={list(data.keys())}")
            self.check("Graph: has edges", "edges" in data,
                        f"keys={list(data.keys())}")
    ...
    def test_entity_list(self):
        ...
        self.check("Entities list: 200", r.status_code == 200, f"status={r.status_code}")
        if r.status_code != 200:
            return
        data = r.json()
        self.check("Entities list: returns list", isinstance(data, list),
                    f"type={type(data).__name__}")
        self.check("Entities list: has entries", len(data) > 0,
                    f"count={len(data)}")
        ...
        if eid:
            r2 = self.client.get(f"{self.api}/entities/{eid}", params={"tenant_id": TENANT})
            self.check("Entity GET: 200", r2.status_code == 200,
                        f"status={r2.status_code}{self._verbose_body(r2)}")
            if r2.status_code == 200:
                edata = r2.json()
                self.check("Entity GET: has canonical_name", bool(edata.get("canonical_name")),
                            f"canonical_name={edata.get('canonical_name')}")
                self.check("Entity GET: has entity_type", bool(edata.get("entity_type")),
    ...
    def test_usage(self):
        ...
        if r.status_code == 200:
            ...
            self.check("Usage: has usage data",
                        isinstance(usage, dict) and len(usage) > 0,
                        f"keys={list(data.keys())}")

    def test_usage_history(self):
        """GET /api/usage/history returns historical usage periods."""
        r = self.client.get(f"{self.api}/usage/history", params={"tenant_id": TENANT, "periods": 3})
        # This endpoint may require org context; 200 or 400 are both valid
        if r.status_code == 400:
            self.skip("Usage history", "requires organization context")
            return
        self.check("Usage history: 200", r.status_code == 200,
                    f"status={r.status_code}{self._verbose_body(r)}")
        if r.status_code == 200:
            data = r.json()
            self.check("Usage history: returns list", isinstance(data, list),
                        f"type={type(data).__name__}")

    def test_audit_log(self):
        """GET /api/audit-log returns audit entries for test tenant."""
        r = self.client.get(f"{self.api}/audit-log", params={"tenant_id": TENANT, "limit": 10})
        self.check("Audit log: 200", r.status_code == 200,
                    f"status={r.status_code}{self._verbose_body(r)}")
        if r.status_code != 200:
            return
        data = r.json()
        self.check("Audit log: returns list", isinstance(data, list),
                    f"type={type(data).__name__}")
        self.check("Audit log: has entries from smoke test writes", len(data) > 0,
                    f"count={len(data)}")
        if data:
            entry = data[0]
            self.check("Audit log: entry has action", bool(entry.get("action")),
                        f"action={entry.get('action')}")
            self.check("Audit log: entry has created_at", bool(entry.get("created_at")),
    ...
    def test_settings(self):
        ...
        r2 = self.client.put(f"{self.api}/settings",
                              params={"tenant_id": TENANT},
                              json={"graph_expand": False})
        self.check("Settings PUT: 200", r2.status_code == 200, f"status={r2.status_code}")
        ...
        if r2.status_code == 200:
            ...
            self.check("Settings: value updated",
                        search_cfg.get("graph_retrieval") is False
                        or isinstance(data2, dict),
                        f"search={search_cfg}")

        # Revert
        self.client.put(f"{self.api}/settings",
                         params={"tenant_id": TENANT},
    ...
    def test_auth_verify(self):
        ...
        if not self.headers.get("X-API-Key"):
            ...
            return
        r = self.client.post(f"{self.api}/auth/verify", json={
        ...
        if r.status_code == 200:
            ...
            self.check("Auth verify: valid response",
                        data.get("valid") is True,
                        f"data={data}")

    def test_mcp_initialize(self):
        """Initialize an MCP session."""
        r = self.client.post(f"{self.base}/mcp/", json={
        ...
        body_has_result = "initialize" in r.text or "protocolVersion" in r.text or has_session
        self.check("MCP: initialize succeeds", r.status_code == 200 and body_has_result,
                    f"status={r.status_code}")
    ...
    def test_list_memories(self):
        ...
        data = r.json()
        self.check("List memories: 200", r.status_code == 200, f"status={r.status_code}")
        self.check("List memories: has items key", isinstance(data, dict) and "items" in data,
                    f"keys={list(data.keys()) if isinstance(data, dict) else type(data).__name__}")
        items = data.get("items", []) if isinstance(data, dict) else []
        self.check("List memories: has our writes", len(items) >= 3,
                    f"count={len(items)}")
        self.check("List memories: has next_cursor field", "next_cursor" in data,
    ...
    def test_document_store(self):
        ...
        # 1. Write a document
        r = self.client.post(f"{self.api}/documents", json={
            "tenant_id": TENANT, "collection": collection,
            "doc_id": "acme-corp",
            "data": {"name": "Acme Corp", "plan": "business", "mrr": 4500, "active": True},
        })
        self.check("Doc write: 200", r.status_code == 200,
                    f"status={r.status_code}{self._verbose_body(r)}")
        if r.status_code == 200:
            self.check("Doc write: has doc_id", r.json().get("doc_id") == "acme-corp",
                        f"doc_id={r.json().get('doc_id')}")

        # 2. Get the document
        r = self.client.get(f"{self.api}/documents/acme-corp",
                            params={"tenant_id": TENANT, "collection": collection})
        self.check("Doc get: 200", r.status_code == 200,
                    f"status={r.status_code}{self._verbose_body(r)}")
        if r.status_code == 200:
            self.check("Doc get: data matches", r.json().get("data", {}).get("mrr") == 4500,
                        f"data={r.json().get('data')}")

        # 3. Write a second document
        self.client.post(f"{self.api}/documents", json={
            "tenant_id": TENANT, "collection": collection,
            "doc_id": "beta-inc",
        ...
        r = self.client.post(f"{self.api}/documents/query", json={
            "tenant_id": TENANT, "collection": collection,
            "where": {"plan": "business"},
        })
        self.check("Doc query: 200", r.status_code == 200,
                    f"status={r.status_code}{self._verbose_body(r)}")
        if r.status_code == 200:
            results = r.json()
            self.check("Doc query: finds 1 business", len(results) == 1,
                        f"count={len(results)}")
            if results:
                self.check("Doc query: correct doc", results[0].get("doc_id") == "acme-corp",
        ...
        r = self.client.post(f"{self.api}/documents", json={
            "tenant_id": TENANT, "collection": collection,
            "doc_id": "acme-corp",
            "data": {"name": "Acme Corp", "plan": "business", "mrr": 6000, "active": True},
        })
        self.check("Doc upsert: 200", r.status_code == 200, f"status={r.status_code}")
        if r.status_code == 200:
            self.check("Doc upsert: mrr updated", r.json().get("data", {}).get("mrr") == 6000,
                        f"mrr={r.json().get('data', {}).get('mrr')}")

        # 6. List collection
        r = self.client.get(f"{self.api}/documents",
                            params={"tenant_id": TENANT, "collection": collection})
        self.check("Doc list: 200", r.status_code == 200, f"status={r.status_code}")
        if r.status_code == 200:
            self.check("Doc list: 2 docs", len(r.json()) == 2,
                        f"count={len(r.json())}")

        # 7. Delete
        r = self.client.delete(f"{self.api}/documents/acme-corp",
                               params={"tenant_id": TENANT, "collection": collection})
        ...
        r = self.client.get(f"{self.api}/documents/acme-corp",
                            params={"tenant_id": TENANT, "collection": collection})
        self.check("Doc get deleted: 404", r.status_code == 404, f"status={r.status_code}")

        # Cleanup remaining
        self.client.delete(f"{self.api}/documents/beta-inc",
                           params={"tenant_id": TENANT, "collection": collection})
    ...
    def test_write_missing_content(self):
        ...
        self.check("Validation: write missing content returns 422",
                    r.status_code == 422,
                    f"status={r.status_code}{self._verbose_body(r)}")
    ...
    def test_search_empty_query(self):
        ...
        self.check("Validation: search empty query returns 422",
                    r.status_code == 422,
                    f"status={r.status_code}{self._verbose_body(r)}")
    ...
    def test_write_oversized_content(self):
        ...
        self.check("Validation: oversized content returns 422",
                    r.status_code == 422,
                    f"status={r.status_code}")

    def test_invalid_memory_id_format(self):
        """GET /api/memories/not-a-uuid -> 422."""
        r = self.client.get(f"{self.api}/memories/not-a-uuid", params={"tenant_id": TENANT})
        self.check("Validation: invalid UUID returns 422",
                    r.status_code == 422,
                    f"status={r.status_code}{self._verbose_body(r)}")
    ...
    def test_soft_delete(self):
        ...
        if not self.memory_ids:
            ...
            return
        mid = self.memory_ids[0]
        r = self.client.delete(f"{self.api}/memories/{mid}", params={"tenant_id": TENANT})
        self.check("Soft delete: 204", r.status_code == 204,
                    f"status={r.status_code}")
    ...
    def test_demo_token(self):
        ...
        if r.status_code == 404:
            ...
            return
        data = r.json()
        self.check("Demo token: returns key", bool(data.get("api_key")),
                    f"api_key={'present' if data.get('api_key') else 'missing'}")
        self.check("Demo token: returns tenant", bool(data.get("tenant_id")),
    ...
    def cleanup(self):
        ...
        for mid in self.memory_ids:
            try:
                self.client.delete(f"{self.api}/memories/{mid}", params={"tenant_id": TENANT})
            except Exception:
                pass
        ...
        for eid in self.entity_ids:
            try:
                self.client.delete(f"{self.api}/entities/{eid}", params={"tenant_id": TENANT})
            except Exception:
                pass
...
def main():
    ...
    parser.add_argument("--verbose", "-v", action="store_true", help="Show response bodies on failure")
    parser.add_argument("--json", dest="json_output", action="store_true", help="JSON output for CI pipelines")
    args = parser.parse_args()

    test = SmokeTest(args.url, args.api_key, verbose=args.verbose, json_output=args.json_output)
    sys.exit(test.run())
