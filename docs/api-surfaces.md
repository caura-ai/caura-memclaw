...
| Skill sharing | REST (`POST /skills/share`, `DELETE /skills/{name}`, `GET /skills`) + MCP (`memclaw_share_skill`, `memclaw_unshare_skill`) | Agents share `SKILL.md` artifacts with the fleet. Storage: `documents` collection `skills` (description auto-embedded for semantic search via `GET /skills?query=`). Distribution: opt-in `install_skill` / `uninstall_skill` fleet commands processed by the plugin's heartbeat handler — writes/deletes `plugin/skills/<name>/SKILL.md` for OpenClaw's native skill discovery to pick up at next workspace open. Default mode is publish-only (catalog) — fleet-wide install/uninstall is opt-in via `install_on_fleet=true` / `unshare_from_fleet=true`. Trust ≥ 1. |
